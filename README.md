...
- Injection:Analyzed SQL Injection (SI) and OS Command Injection (OC) risks.
- Broken Access Control: Diagnosed insufficient authentication (IA) and authorization (IN).
- Cross-Site Attacks: Verified Stored/Reflected XSS (XS) and CSRF (CF) vulnerabilities.
- File Handling: Tested for malicious Web Shell uploads (FU) and Path Traversal during downloads (FD).
- Information Leakage: Identified exposure of sensitive data in source code and verbose error page
